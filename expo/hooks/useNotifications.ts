 ...
 *
 * Served through React Query so the feed header (unread dot) and the
 * notifications screen share one cache instead of double-fetching.
 */
...
async function fetchNotifications(userId: string): Promise<AppNotification[]> {
  // Get my profile (id-filtered — profiles are publicly readable, so an
  // unfiltered maybeSingle() would fail with "multiple rows returned").
  ...
  // Tips received (if creator) — batch the fan profiles in one query
  if (me.is_creator) {
    ...
    const { data: fans } = fanIds.length > 0
      ? await supabase.from("profiles").select("id, name, handle, avatar_url").in("id", fanIds)
      : { data: [] as { id: string; name: string | null; handle: string | null; avatar_url: string | null }[] };

    const fanMap = new Map<string, { name: string | null; handle: string | null; avatar_url: string | null }>();
    for (const f of fans ?? []) fanMap.set(f.id, f);

    for (const tip of tips ?? []) {
      const fan = fanMap.get(tip.fan_id);
      notices.push({
        id: `tip-${tip.id}`,
