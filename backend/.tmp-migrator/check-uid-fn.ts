...
async function main() {
  ...
  const { rows: fns } = await client.query(`
    select routine_name, routine_definition, security_type, security_definer
  ...
  for (const f of allFns) console.log(`  ${f.routine_name} | returns ${f.data_type} | ${f.security_type}`);
